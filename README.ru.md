...
- Случайные автоответы ограничены только свежими сообщениями, поэтому бот не некропостит в старые ветки
...
- Обычные ссылки `https://...` больше не вызывают ложные срабатывания `ai-injection-guard`, если это просто ссылка
- Нормальные ZWJ-эмодзи вроде `🤷‍♂️` больше не вызывают ложные срабатывания `unicode_smuggling`
...
[groq]
model = openai/gpt-oss-120b  # текущая production text model на Groq
...
| `bot_metrics`    | Персистентные глобальные счётчики для статистики суперадмина |
| `chat_metrics`   | Счётчики активности по чатам для дашбордов суперадмина       |
...
| Команда          | Описание                                                  |
|------------------|-----------------------------------------------------------|
| `/sa_chats`      | Группирует активные пространства по группам/ЛС/каналам и показывает активность, историю, настройки и счётчики инъекций |
| `/sa_stats`      | Группированный дашборд: runtime, разрез по типам, трафик, топ пространств, размеры БД |
| `/sa_broadcast`  | Отправить сообщение во все активные группы (диалог)       |
| `/cancel`        | Прервать текущий broadcast                                |

`/sa_chats` пытается подтянуть live-данные из Telegram по `chat_id` (имя/название и `@username`) с fallback на БД. Для каждого чата также показываются tracked incoming, ответы, строки истории, количество пользователей, счётчики инъекций и текущие настройки.
