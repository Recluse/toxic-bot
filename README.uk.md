...
- Випадкові автовідповіді обмежені лише свіжими повідомленнями, тому бот не некропостить у старі гілки
...
- Адаптивна мова відповіді: якщо повідомлення явно повністю іншою однією мовою, бот відповідає цією мовою
- Префільтр prompt injection до будь-якого виклику LLM (`<system>`, `</system>`, `<\\system>` і перевірки `ai-injection-guard`)
- Санітизація контексту перед викликом LLM: історія й reply-chain фільтруються на інʼєкції, небезпечні фрагменти вирізаються
- Захист за тегами також покриває варіанти з `admin` (`<admin>`, `</admin>`, `<\\admin>`) включно з regex і escaped-формами
- Звичайні посилання `https://...` більше не викликають хибні спрацювання `ai-injection-guard`, якщо це просто посилання
- Нормальні ZWJ-емодзі на кшталт `🤷‍♂️` більше не викликають хибні спрацювання `unicode_smuggling`
- Окремий лог інʼєкцій: `logs/prompt_injection_events.log` з повним payload чату/користувача/повідомлення
- PM-сповіщення суперадміну при кожному спрацюванні інʼєкційного фільтра з джерелом і детальною причиною
- Інʼєкційні повідомлення у випадковому груповому потоці тихо ігноруються; явна реакція лишається в ЛС, командах і прямих реплаях боту
...
[groq]
model = openai/gpt-oss-120b  # поточна production text model на Groq
...
| `bot_metrics`    | Персистентні глобальні лічильники для статистики суперадміна   |
| `chat_metrics`   | Лічильники активності по чатах для дашбордів суперадміна       |
...
| Команда          | Опис                                                      |
|------------------|-----------------------------------------------------------|
| `/sa_chats`      | Групує активні простори за групами/ЛС/каналами і показує активність, історію, налаштування та лічильники інʼєкцій |
| `/sa_stats`      | Згрупований дашборд: runtime, розріз за типами, трафік, топ просторів, розміри БД |
| `/sa_broadcast`  | Надіслати повідомлення до всіх активних груп (діалог)     |
| `/cancel`        | Перервати поточний broadcast                              |

`/sa_chats` намагається підтягнути live-дані з Telegram за `chat_id` (ім'я/назва та `@username`) з fallback на БД. Для кожного чату також показуються tracked incoming, відповіді, рядки history, кількість користувачів, лічильники інʼєкцій і поточні налаштування.
